...
# Creates: 2025-11-30-jtnovellis-try
...
**Arguments:**
- `path` (optional): Override default tries directory (absolute path recommended)

**Behavior:**
- Detects current shell (bash/zsh or fish) via `$SHELL` environment variable
- Outputs appropriate function definition to stdout
- Function wraps `try exec` and evals output
- Embeds the full path to the `try` binary at init time
...
Automatically append the shell function to the user's shell config file.
...
**Behavior:**
- Detects shell and RC file location (bash: ~/.bashrc or ~/.bash_profile, zsh: ~/.zshrc, fish: ~/.config/fish/config.fish)
- Appends the `init` snippet with a `# try shell integration` marker
