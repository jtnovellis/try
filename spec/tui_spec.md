...
2. Preserve ANSI escape sequences (don't split `\x1b[...m` codes)
...
3. Skip ANSI escape sequences (`\x1b[...m`) - don't count them
4. Count visible characters until limit reached
5. Append ellipsis (`…`)

```
Input:  "2025-11-29-\x1b[1;33mvery\x1b[39m-long-project-name" (29 visible chars)
Max:    20 chars
Output: "2025-11-29-\x1b[1;33mvery\x1b[39m-lon…" (19 visible + ellipsis)
```

ANSI escape sequences are preserved intact - never split a color code pair.
...
| Ctrl-D | Mark directory for deletion |
| Ctrl-R | Rename selected directory |
| Ctrl-G | Graduate (promote) selected directory to project |
...
Selection can result in multiple action types:

| Action | Trigger | Result |
|--------|---------|--------|
| CD | Select existing directory | Navigate to directory |
| MKDIR | Select "[new]" entry | Create and navigate to new directory |
| DELETE | Press Ctrl-D, then Enter + YES | Delete marked directories |
| RENAME | Press Ctrl-R, type new name, Enter | Rename directory |
| ASCEND | Press Ctrl-G, confirm destination, Enter | Graduate try to project directory |
...
- Marked entries display with a dark red background (ANSI 256-color bg 52)
...
```sh
cd '/path/to/tries' && \
  test -d 'dir-name' && rm -rf 'dir-name' && \
